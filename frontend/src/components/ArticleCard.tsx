...
export default function ArticleCard({ article, categories, selectable, selected, onSelect }: Props) {
  ...
  if (selectable) {
    return (
      <div
        ...
        className={`group block bg-surface rounded-xl overflow-hidden ring-1 cursor-pointer transition-all duration-200 ${selected ? 'ring-amber-500 ring-2' : 'ring-rim hover:ring-amber-500/50'}`}
      >
        {inner}
      </div>
    );
  ...
  return (
    <Link to={`/articles/${article.slug}`} className="group block bg-surface rounded-xl overflow-hidden ring-1 ring-rim hover:ring-amber-500/50 transition-all duration-200">
      {inner}
    </Link>
  );
}
