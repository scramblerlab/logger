...
export default function ImportPage() {
  ...
  const handleAnalyze = async () => {
    ...
    setAnalysis(null);
    try {
      const result = await api.importer.analyze(url.trim());
      setAnalysis(result);
    } catch (e) {
  ...
  const handleImport = () => {
    ...
    const importUrl = `/api/import/run`;
    fetch(importUrl, {
      method: 'POST',
        ...
        reader.read().then(({ done: streamDone, value }) => {
          ...
          for (const line of lines) {
            ...
            try {
              ...
              setProgress(ev);
              if (ev.saved) setLog((prev) => [`✓ ${ev.saved}`, ...prev].slice(0, 100));
              if (ev.error) setLog((prev) => [`✗ ${ev.error}`, ...prev].slice(0, 100));
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <Link to="/" className="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-indigo-600 mb-6">← 一覧に戻る</Link>
      <h2 className="text-2xl font-bold text-gray-900 mb-2">一括インポート</h2>
      ...
      {analysis && (
        <div className="bg-indigo-50 rounded-xl p-5 mb-6 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-indigo-900">分析結果</h3>
            <span className="text-xs text-indigo-500 bg-indigo-100 px-2 py-0.5 rounded">{analysis.method}</span>
          </div>
          <p className="text-sm text-gray-700">約 <span className="font-bold text-indigo-700">{analysis.article_count}</span> 件の記事を検出</p>
          ...
          <button
            ...
            disabled={running}
            className="w-full mt-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-medium py-2.5 rounded-lg transition-colors text-sm"
          >
      ...
      {done && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6 text-sm text-green-700 font-medium">
          ...
          <Link to="/" className="ml-3 text-indigo-600 hover:underline">一覧を見る →</Link>
        </div>
      )}

      {log.length > 0 && (
        <div className="bg-gray-900 text-gray-300 rounded-xl p-4 text-xs font-mono max-h-64 overflow-y-auto space-y-0.5">
          {log.map((l, i) => (
